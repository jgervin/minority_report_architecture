...
- **mras-composer — runtime (render seam, preview, trigger).**
  ...
  - `/trigger`: for an **identified** viewer, selector picks the active custom-component ad; merge
    `default_props + { personalized_field: name } + base-meta`; render warm via the seam;
    output-conformance check; `assemble(base, [(audio,250)], trigger_id, overlay_inserts=…)`; WS
    `play`. **On overlay render/conformance failure, fall back to the no-overlay clip** (voice still
    plays — M3 behavior; never drop an identified viewer's ad). A **not-identified** trigger returns
    `standard` with **no broadcast** (idle pool keeps looping).

- **selector — custom-ad selection.** **Identified** viewer (vision matched the embedding in qdrant →
  composer finds the unblocked identity row in postgres) → **always** the `is_active`
  custom-component ad, `personalized_field` filled from the name (voice + overlay). **Not identified**
  (no uuid / `is_new_visitor` / not found / blocked) → `standard` branch returns **without a WS
  broadcast**, so **no one-shot playback happens and the kiosk's idle rotation keeps looping the
  pool** (there is no triggered "standard.mp4" play). If no `is_active` custom ad exists, an
  identified viewer falls back to the M3 personalized path (voice + name overlay). Rotation/targeting
  beyond "active" is deferred.
...
5. **Run live**: an **identified** viewer triggers selection of the active custom ad → warm render
   with the viewer's name → composite → kiosk plays a one-shot. Unidentified viewers trigger nothing —
   the idle pool keeps looping.
...
5. **mras-composer**: `/trigger` custom-ad selection wiring (selector reads ads; merge props; render;
   fallback). Tests: identified viewer → assemble called with the custom overlay + WS play;
   conformance/sidecar failure → no-overlay fallback (voice still plays); not-identified → `standard`,
   no broadcast (idle pool loops).
